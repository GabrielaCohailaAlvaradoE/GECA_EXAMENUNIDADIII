...
<head>
    <meta charset="UTF-8">
    <title>Fase 7 · Reporte de firmas</title>
    <style>
...
<body>
    <header>
        <h1>Fase 7 · Reporte del Jefe</h1>
        <p>Visualiza quiénes firmaron sus boletas y quiénes están pendientes.</p>
    </header>
    <main>
        ...
        <div class="card">
            <div style="display:flex; justify-content: space-between; align-items: center; gap: 1rem; flex-wrap: wrap;">
                <div>
                    ...
                    <p class="muted">Revisa el estado de firmas por fase y por empleado.</p>
                </div>
                <div class="actions">
        ...
        <div class="card">
            ...
            <form method="get" action="${pageContext.request.contextPath}/reporte-firmas">
        ...
        <c:if test="${not empty planillaActual}">
            <div class="card">
                <h3>Avance de firmas por fase</h3>
        ...
        <c:if test="${not empty detalles}">
            <div class="card">
                <h3>Empleados con boleta pendiente</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Empleado</th>
                            <th>Neto</th>
                            <th>Estado firma empleado</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:forEach var="det" items="${detalles}">
                            <c:set var="estadoEmpleado" value="PENDIENTE"/>
                            <c:forEach var="fi" items="${firmas}">
                                <c:if test="${fi.fase eq 'FASE6_EMPLEADO' && fi.usuario.idUsuario == det.empleado.idEmpleado}">
                                    <c:set var="estadoEmpleado" value="${fi.estado}"/>
                                </c:if>
                            </c:forEach>
                            <tr>
